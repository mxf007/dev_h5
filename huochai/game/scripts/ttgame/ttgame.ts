...
export class TtgamePlugin implements plugins.Command {
    ...
    async onFile(file: plugins.File) {
        if (file.extname == '.js') {
            ...
            if (filename == 'libs/modules/egret/egret.js' || filename == 'libs/modules/egret/egret.min.js') {
                ...
                if (filename == "libs/modules/mylib/mylib.js" || filename == 'libs/modules/mylib/mylib.min.js') {
                    content += ";window.mylib = mylib;"
                }
                ...
                if (filename == 'main.js') {
                    content += "\n;window.Main = Main;window.OneStateButton =OneStateButton;"
                }
                file.contents = new Buffer(content);
            }
        }
    ...
    async onFinish(pluginContext: plugins.CommandContext) {
        ...
        if(!fs.existsSync(gameJSPath)) {
            console.log(`${gameJSPath}不存在，请先使用 Launcher 发布字节跳动小游戏`);
            return;
        }
